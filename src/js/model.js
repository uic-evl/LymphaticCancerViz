...
function Patients() {
  ...
  self.changeCurrentPatient = function (newValue) {
    if(!newValue) return;
  ...
  function setup2WayBindings(){
    ...
    self.currentPatient.subscribe(self.changeCurrentPatient);
...
(function () {
  ...
  function init() {
    ...
    App.utils.readFiles(function (error, weighted,weighted_w6, weighted_n6)
    {
      ...
      let patientQuery = App.utils.getQueryVariable("id");
      ...
      let model = new Patients();
        ...
        if(patientQuery) {
            let patient_obj = _.find(App.self.patients(), {id: parseInt(patientQuery)});
            model.changeCurrentPatient(patient_obj);
        }
